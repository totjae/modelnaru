...
| ------------- | -------------------------------------------- | --------------------------------- | ------------ | --------------- |
| `llm-gateway` | `Authorization: Bearer`                      | `/models?exclude_deprecated=true` | 통과         | 미검증          |
| `openai`      | `Authorization: Bearer`                      | `/models`                         | 통과         | 미검증          |
...
- Ubuntu 실제 LLM Gateway 키 등록, 모델 동기화와 재시작 후 복호화 smoke test는 배포 후 별도로 기록한다.
