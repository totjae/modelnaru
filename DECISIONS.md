...
### ADR-020: 채팅 설정을 기본 닫힘 모달로 전환

- 상태: 확정
- 결정일: 2026-07-23
- 결정: 채팅의 상시 오른쪽 설정 열을 제거하고 기본 닫힘 상태의 중앙 모달로 전환한다. 데스크톱은 최대 860px, 모바일은 전체 화면을 사용하며 본문 스크롤과 고정 하단 동작을 분리한다.
- 이유: 설정보다 대화 본문을 더 자주 사용하는 화면에서 상시 설정 열이 본문 너비를 과도하게 줄이기 때문이다.
- 영향: 상단에는 현재 모델과 설정 버튼을 표시한다. 미저장 상태로 취소·바깥 클릭·Esc를 사용하면 확인 후 모달을 열기 전 모델·파라미터 상태로 복원한다.

### ADR-021: 대화와 분리된 사용량 원장과 관리자 기본 Usage 화면

- 상태: 확정
- 결정일: 2026-07-23
- 결정: 관리자 로그인 후 `Usage`를 기본 화면으로 표시하고 기존 기능은 사용자·게스트·프로바이더·장기기억 탭으로 분리한다. 생성·재생성 요청은 대화 본문과 분리된 `usage_events`에 주체·모델 snapshot, 상태, 입출력 token과 처리 시간만 기록한다.
- 이유: 메시지에서 직접 집계하면 사용자·대화 삭제 시 과거 통계가 함께 사라지고, 모든 관리 기능을 한 페이지에 나열하면 필요한 설정을 찾기 어렵기 때문이다.
- 대안: 메시지 table을 실시간 집계, 일별 aggregate만 저장, 외부 metrics 시스템만 사용.
- 영향: 사용자와 대화를 삭제해도 사용량 원장은 유지되며 메시지 FK만 `NULL`이 된다. 비용은 Provider별 가격표가 확정되기 전까지 계산하지 않고, 10분부터 30일까지의 상대 기간별 token·요청·상태만 표시한다.

### ADR-022: 대화방별 기본 모델과 생성 파라미터 저장

- 상태: 확정
- 결정일: 2026-07-23
- 결정: 설정 모달에서 선택한 기본 모델과 생성 파라미터를 `conversations` 행에 저장하고 대화 UUID별로 복원한다. 각 assistant 메시지에는 실제 호출 모델과 검증된 파라미터 snapshot을 별도로 유지한다.
- 이유: Web의 단일 상태만 사용하면 대화를 전환할 때 직전 대화 설정이 다른 대화에 남고, 마지막 assistant 메시지만으로는 아직 호출하지 않은 설정 변경을 복원할 수 없기 때문이다.
