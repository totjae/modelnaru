...
현재 OpenAI Chat Completions 호환, Anthropic Messages와 Gemini GenerateContent SSE adapter, 공통 text·usage·done·error event, 메시지 최종 상태 저장, upstream 취소와 답변 재생성 분기가 구현됐다. 실제 credential 기반 protocol별 smoke test, Responses API, 자동 요약과 파일 입력은 후속 검증·구현 범위다.
