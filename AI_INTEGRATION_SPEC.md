...
관리자 요약 설정과 일반 채팅은 `provider-manager-v1.10.0.js`에서 확인한 동일한 중앙 parameter policy를 사용한다. 각 선택 항목은 `Provider 기본값`과 `직접 설정`을 명시적으로 구분하며 빈 입력을 기본값의 의미로 사용하지 않는다. 최대 출력은 관리자 설정과 모델 상한 중 작은 값으로 제한한다. 모델·prompt·parameter 설정을 저장할 때 version을 증가시켜 설정이 다른 기존 요약을 재사용하지 않는다.

- OpenAI 호환 일반 모델: `temperature`, `topP`, frequency·presence penalty, seed, stop sequence
- GPT-5·o 계열 추론 모델: sampling 대신 `reasoningEffort`, `verbosity`를 표시하고 `max_completion_tokens`를 사용
