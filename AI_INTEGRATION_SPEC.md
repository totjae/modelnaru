...
관리자는 요약 요청의 `temperature`, `topP`와 최대 출력 token을 지정할 수 있다. sampling 값이 비어 있으면 Provider 기본값을 사용하고 최대 출력은 관리자 설정과 모델 상한 중 작은 값으로 제한한다. 모델·prompt·parameter 설정을 저장할 때 version을 증가시켜 설정이 다른 기존 요약을 재사용하지 않는다.
