...
- reasoning 또는 thinking을 활성화해 temperature·top_p와 함께 쓸 수 없으면 해당 항목을 숨기지 않고 비활성 상태와 이유를 표시하며 서버 요청에서는 제거한다.
...
관리자 요약 설정과 일반 채팅은 `provider-manager-v1.10.0.js`에서 확인한 동일한 중앙 parameter policy를 사용한다. 각 항목은 용도 설명, Provider 기본 동작, `직접 설정` 체크박스와 설정 입력을 항상 같은 위치에 표시하며 빈 입력을 기본값의 의미로 사용하지 않는다. 최대 출력은 관리자 설정과 모델 상한 중 작은 값으로 제한한다. 모델·prompt·parameter 설정을 저장할 때 version을 증가시켜 설정이 다른 기존 요약을 재사용하지 않는다.

- OpenAI 호환 일반 모델: `temperature`, `topP`, frequency·presence penalty, seed, stop sequence
- GPT-5·o 계열 추론 모델: 기존 sampling 항목과 `reasoningEffort`, `verbosity`를 함께 표시한다. reasoning effort를 직접 설정한 동안 상호 배타적인 temperature·topP는 비활성화하고 요청에서 제거하며 `max_completion_tokens`를 사용한다.
- Anthropic: temperature 상한 1, `topP`, `topK`, stop sequence, thinking budget·표시, output effort. thinking이 활성화되면 상호 배타적인 sampling 값을 제거
- Gemini: `temperature`, `topP`, `topK`, frequency·presence penalty, seed, stop sequence, thinking budget·level
- NovelAI: OpenAI 호환 sampling에 `topK`와 thinking budget을 추가
- Vertex AI, Bedrock, Copilot: 모델 계열과 실제 route에 따라 Anthropic·Gemini·OpenAI 정책을 선택
- 나머지 registry template Provider: 명시적 override가 없으면 OpenAI 호환 정책을 사용

현재 tokenizer가 없는 모델도 일관되게 보호하기 위해 system prompt와 메시지 본문의 Unicode 문자 수를 1문자당 최대 1 token으로 보수적으로 계산한다. 최근 컨텍스트는 적용 한도의 약 55% 안에서 보존하고 나머지 오래된 prefix를 요약한다. 활성 경로에 기존 요약의 최종 메시지가 포함되고 요약 모델·프롬프트 버전이 같으면 해당 요약 뒤의 메시지만 붙여 재사용한다.

요약 모델이 비활성화되었거나 요청에 실패하면 원본을 무단 절단하지 않고 본 요청을 중단한다. 사용자에게 컨텍스트 범위를 줄이거나 다시 시도하도록 안내한다.

이 단계는 시간순으로 오래된 구간을 압축하는 방식이며 요약을 근거로 원본 메시지를 검색해 재삽입하지 않는다. 따라서 embedding이나 vector database는 필요하지 않다. 향후 대화·문서 전체에서 관련 원문을 검색하는 RAG를 도입할 때 embedding을 별도 기능으로 추가한다.
...
- Provider가 전혀 지원하지 않는 파라미터는 표시하지 않는다. 선택한 기능과 충돌해 일시적으로 적용할 수 없는 기존 파라미터는 숨기지 않고 비활성 상태와 이유를 표시하며 서버도 요청에서 제거한다.
...
현재 OpenAI Chat Completions 호환, Anthropic Messages와 Gemini GenerateContent SSE adapter, 공통 text·usage·done·error event, 메시지 최종 상태 저장, upstream 취소, 답변 재생성 분기, 자동 컨텍스트 요약과 이미지 입력 변환이 구현됐다. 이미지는 각각 OpenAI `image_url`, Anthropic base64 image block, Gemini `inline_data`로 변환한다. 실제 credential 기반 protocol별 이미지 smoke test와 Responses API는 후속 검증·구현 범위다.
...
## 20. Registry Provider 실행 범위와 채팅 기본값

- 2026-07-23 기준 Provider Manager 원격 registry의 32개 API template은 모델 조회 또는 고정 모델 snapshot을 통해 등록할 수 있다.
- 모든 registry template의 기본 채팅 protocol은 해당 template의 OpenAI 호환 `formats.openai` endpoint다.
- 원격 registry를 실행 중 자동 반영하지 않는다. 검수한 snapshot만 배포 코드에 포함한다.
