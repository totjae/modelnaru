...
type ChatEvent =
  ...
  | { type: "usage"; inputTokens?: number; outputTokens?: number; cachedInputTokens?: number; reasoningTokens?: number }
...
1. 로그인과 CSRF 보호를 확인한다.
2. 대화방 소유권을 확인한다.
3. 사용자가 해당 모델을 사용할 권한이 있는지 확인한다.
...
8. 선택한 제공자의 활성 자격증명 또는 키 그룹을 선택한다.
9. 모델 메타데이터에 따라 요청 형식을 선택한다.
10. provider connection과 protocol engine이 외부 요청을 만든다.
11. 외부 응답을 공통 이벤트로 변환하여 브라우저에 스트리밍한다.
12. 사용자가 중단하면 서버의 upstream 요청도 `AbortController`로 취소한다.
13. 최종 텍스트, 사용량, 처리 시간, 중단 사유와 오류 코드를 저장한다.
