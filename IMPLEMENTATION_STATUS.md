...
| 데이터베이스         | 구현 완료 | users·sessions 적용 확인, audit_logs migration은 Ubuntu 적용 필요       |
| 인증·session         | 구현 완료 | 고정 관리자 TOTP·session·CSRF·로그아웃, Ubuntu HTTPS login 성공 확인    |
| 사용자 관리          | 구현 완료 | 관리자 CRUD·비밀번호·session 폐기·감사 기록·Web UI, Ubuntu 검증 필요    |
