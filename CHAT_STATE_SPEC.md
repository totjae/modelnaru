...
- user 메시지는 같은 대화에 속한 텍스트·PDF attachment metadata를 가질 수 있다. 원본 본문은 그대로 저장하며 AI context를 만들 때만 현재 첨부와 후속 포함 첨부의 추출문을 합성한다. PDF 추출문은 페이지 구분자를 유지하므로 모델이 근거 페이지를 구별할 수 있다.
