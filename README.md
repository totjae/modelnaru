...
프로젝트 기반, 고정 관리자 인증, 관리자 사용자 관리와 일반 사용자 로그인이 구현되었습니다. pnpm monorepo, 역할별 로그인·사용자 관리 Web, NestJS API, 공통 `config.yaml` loader·validator, `apichat-admin`, PostgreSQL migration, Argon2id·TOTP·서버 session·CSRF, 사용자 변경 감사 원장, PostgreSQL·Valkey와 단일 gateway Compose 구성이 준비되어 있습니다. Provider와 대화 기능은 후속 단계에서 순서대로 연결합니다.
