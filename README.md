...
프로젝트 기반, 역할별 인증·사용자 관리, Provider 등록과 접근 정책 기반이 구현되었습니다. pnpm monorepo, 관리자 Provider 카탈로그·API 키 등록·모델 관리 Web, 사용자·게스트 모델 allowlist와 일일 호출량 예약, 공유 코드 기반 독립 게스트 session, NestJS API, AES-256-GCM 자격증명 저장, PostgreSQL migration, Argon2id·TOTP·서버 session·CSRF, 감사 원장, PostgreSQL·Valkey와 단일 gateway Compose 구성이 준비되어 있습니다. 다음 단계에서 실제 AI 요청과 대화 저장 기능에 권한·할당량을 연결합니다.
