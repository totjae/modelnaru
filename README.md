...
프로젝트 기반, 역할별 인증·사용자 관리, Provider 등록·모델 권한, 대화 저장·분기·자동 요약, 사용량 집계와 텍스트·PDF·이미지 첨부 처리가 구현되었습니다. PDF는 기본 100페이지까지 텍스트 레이어를 추출하며 스캔 PDF는 서버 로컬 Tesseract로 한국어·영어 OCR을 수행합니다. JPEG·PNG·WebP는 이미지 입력이 허용된 OpenAI·Anthropic·Gemini 계열 모델에 직접 전달합니다. 첨부파일 보관 기간·만료 원본·cascade 삭제·고아 파일 정리도 자동화했습니다.
