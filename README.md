...
프로젝트 기반과 고정 관리자 인증이 구현되었습니다. pnpm monorepo, Next.js 관리자 로그인 화면, NestJS API, 공통 `config.yaml` loader·validator, `apichat-admin`, PostgreSQL migration, Argon2id·TOTP·서버 session·CSRF, PostgreSQL·Valkey와 단일 gateway Compose 구성이 준비되어 있습니다. 일반 사용자 관리·provider·대화 기능은 후속 단계에서 순서대로 연결합니다.
