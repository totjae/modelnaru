...
프로젝트 기반, 역할별 인증·사용자 관리와 Provider 등록 기반이 구현되었습니다. pnpm monorepo, 관리자 Provider 카탈로그·API 키 등록·모델 관리 Web, NestJS API, AES-256-GCM 자격증명 저장, PostgreSQL migration, Argon2id·TOTP·서버 session·CSRF, 감사 원장, PostgreSQL·Valkey와 단일 gateway Compose 구성이 준비되어 있습니다. 사용자별 모델 권한, 실제 AI 요청과 대화 기능은 후속 단계에서 순서대로 연결합니다.
